...
// A department or programme reference as it comes back from the API —
// populated (an object) on reads, but we keep the string case handled
// too in case an endpoint ever returns it unpopulated.
...
// ======================================================
// DATA — staff roles stay as a fixed list (not tied to a
// collection); department & programme are now fetched live.
// ======================================================
...
function refName<T extends { name: string }>(
  ref: Ref<T>,
  fallback: string
): string {
...
function refId<T extends { _id: string }>(
  ref: Ref<T>
): string {
  if (!ref) return "";
...
function mapApiStaffToStaffMember(
  ...
  return {
    ...
    staffType: normalizeStaffType(
      staff.staffType || staff.staffRole
    ),

    department: refName(staff.department, "Not assigned"),

    programme: isHOD
      ? "Department-wide (HOD)"
      : refName(staff.programme, "Not assigned"),
...
export default function StaffRegistration() {
  ...
  const fetchStaff = async () => {
    try {
      ...
      const mappedStaff = response.staff.map(mapApiStaffToStaffMember);
  ...
  // ======================================================
  // FETCH PROGRAMMES FOR A DEPARTMENT (cascading dropdown)
  // ======================================================

  const fetchProgrammesForDepartment = async (
    departmentId: string
  ) => {
    if (!departmentId) {
      setProgrammes([]);
    ...
    try {
      ...
      const response = await apiRequest<ProgrammeListResponse>(
        `/programmes?department=${departmentId}&status=Active`
      );
  ...
  // ======================================================
  // LOAD STAFF + DEPARTMENTS ON PAGE OPEN
  // ======================================================
  ...
  const handleDepartmentChange = (departmentId: string) => {
    updateForm("departmentId", departmentId);
    updateForm("programmeId", "");
    fetchProgrammesForDepartment(departmentId);
  };

  const handleHODToggle = (checked: boolean) => {
    updateForm("isHOD", checked);
    if (checked) {
      updateForm("programmeId", "");
    }
  };
  ...
  const departmentCount = new Set(
    staff
      .filter((member) => member.department !== "Not assigned")
  ...
  const filteredStaff = useMemo(() => {
    ...
    return staff.filter(
      (member) =>
        ...
        member.programme.toLowerCase().includes(query) ||
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (
      ...
      !form.role ||
      !form.password ||
      (!form.isHOD && !form.programmeId)
    ) {
      setErrorMessage(
        form.isHOD
          ? "Please complete all required fields, including the staff password."
          : "Please complete all required fields, including a programme and the staff password."
      );
    ...
    try {
      ...
      const response = await apiRequest<RegisterStaffResponse>(
        "/staff/register",
        {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            programme: form.isHOD ? undefined : form.programmeId,
      ...
      setSuccessMessage(
        `${response.message} Staff ID: ${
          response.staff.staffId || response.staff._id
        }`
      );
    ...
    <div className="min-h-full space-y-6 bg-slate-50 p-4 md:p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Register and onboard lecturers, academic staff,
        ...
        <CardContent className="p-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search by name, staff ID, email, department, programme or role..."
      ...
      <Card className="overflow-hidden border-none bg-white shadow-sm ring-1 ring-slate-200">
        <div className="border-b border-slate-200 px-5 py-4">
          ...
          <p className="mt-1 text-xs text-slate-500">
            Staff members registered in the administration portal.
          </p>
        </div>
                ...
                <tr>
                  <td colSpan={7} className="px-5 py-16 text-center">
                    ...
                    <p className="mt-1 text-xs text-slate-400">
                      Try another search or register a new staff member.
                    </p>
                  </td>
                </tr>
              ) : (
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {member.isHOD && (
                              <span
                                ...
                                className="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-bold text-indigo-700"
                              >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-300">
                    Create a staff profile and begin onboarding.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-6 p-6">
                ...
                <div className="border-t border-slate-200 pt-6">
                  <div className="mb-4">
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Assign the staff member to the appropriate
                      academic or administrative structure.
                    </p>
                  </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 p-3 md:col-span-2">
                      <input
                        type="checkbox"
                        checked={form.isHOD}
                        onChange={(event) =>
                          handleHODToggle(event.target.checked)
                        }
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          HODs oversee the whole department rather
                          than a single programme, so the programme
                          field below won't apply to them.
                        </p>
                      </div>
                    </label>

                    {/* PROGRAMME — hidden for HODs */}
                    ...
                    {!form.isHOD && (
                      <div className="md:col-span-2">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            updateForm("programmeId", event.target.value)
                          }
                          disabled={
                            !form.departmentId || loadingProgrammes
                          }
                          className="h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-[#006dcc] disabled:bg-slate-50 disabled:text-slate-400"
                        >
                          <option value="">
                            {!form.departmentId
                  ...
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-5 text-blue-700">
                        The staff record will be saved directly to
                        the database and assigned a unique staff ID
                        by the backend.
                      </p>
                    </div>
                  </div>
