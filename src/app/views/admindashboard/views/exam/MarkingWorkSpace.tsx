...
export default function MarkingWorkspace({
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    img.onload = () => {
      ...
      // Scale image to fit canvas width while preserving aspect ratio
  ...
  const handleLoadAnswer = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(
        `${apiUrl}/api/get-theory-answer-by-name/className/${selectedClassName}/student/${encodeURIComponent(selectedStudent)}/subject/${selectedSubject}`,
        { headers: authHeaders() },
      );
      const answers = data?.theoryAnswer?.answers;
      if (answers && Object.keys(answers).length > 0) {
        setTheoryAnswer(answers);
      } else {
        setTheoryAnswer(null);
        setAnswerError("No submitted answer found for this student/subject.");
      }
    } catch (err: any) {
      setTheoryAnswer(null);
    ...
    Object.entries(theoryAnswer).forEach(([qId, answer], idx) => {
      ...
      for (const word of words) {
        ...
        if (ctx.measureText(test).width > 760) {
          ...
          y += 18;
          line = word + " ";
          if (y > canvasRef.current.height - 30) break;
        } else {
          line = test;
        }
      }
  ...
  const undo = () => {
    ...
    const canvas = canvasRef.current;
    if (history.length === 0) {
      // No history — redraw the uploaded image if any, else clear
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      if (uploadedImage) {
      ...
      <header className="z-30 flex min-h-14 shrink-0 flex-wrap items-center gap-2 border-b bg-white px-3 py-2 shadow-sm sm:min-h-[4.5rem] sm:px-4 lg:h-20 lg:flex-nowrap lg:gap-4">
        ...
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2 overflow-x-auto py-1 sm:flex-nowrap lg:gap-2">
          ...
          <Select value={selectedStudent} onValueChange={setSelectedStudent}>
            ...
            <SelectContent>
              {students.map((s) => {
                const name = String(s.studentName || s.name || "Student");
                return (
                  <SelectItem key={String(s._id)} value={name}>
                    {name} ({String(s.AdmNo || "")})
                  </SelectItem>
                );
              })}
          ...
          {mode === "offline" && (
            <>
              ...
              <Button
                ...
                <UploadCloud size={16} /> Upload Answer Sheet
              </Button>
            </>
          )}
